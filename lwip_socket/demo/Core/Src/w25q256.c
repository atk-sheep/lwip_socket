...
u16 W25Q256_TYPE=W25Q256;	//默认是W25Q256
u8 W25Q256_QPI_MODE=0;		//QSPI模式标志:0,SPI模式;1,QPI模式.
...
{  
	...
	if(W25Q256_QPI_MODE)QSPI_Send_CMD(command,0,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE,QSPI_ADDRESS_8_BITS,QSPI_DATA_4_LINES);	//QPI,写command指令,地址为0,4线传数据_8位地址_无地址_4线传输指令,无空周期,1个字节数据
...
//写W25Q256状态寄存器
void W25Q256_Write_SR(u8 regno,u8 sr)   
{   
    u8 command=0;
    switch(regno)
    {
...
{
	if(W25Q256_QPI_MODE)QSPI_Send_CMD(W25X_WriteEnable,0,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE,QSPI_ADDRESS_8_BITS,QSPI_DATA_NONE);	//QPI,写使能指令,地址为0,无数据_8位地址_无地址_4线传输指令,无空周期,0个字节数据
...
{  
	if(W25Q256_QPI_MODE)QSPI_Send_CMD(W25X_WriteDisable,0,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE,QSPI_ADDRESS_8_BITS,QSPI_DATA_NONE);//QPI,写禁止指令,地址为0,无数据_8位地址_无地址_4线传输指令,无空周期,0个字节数据
...
void W25Q256_Read(u8* pBuffer,u32 ReadAddr,u16 NumByteToRead)   
{ 
...
void W25Q256_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	W25Q256_Write_Enable();					//写使能
...
//CHECK OK
void W25Q256_Write_NoCheck(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)   
{ 			 		 
	u16 pageremain;	   
...
void W25Q256_Write(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)   
{ 
	u32 secpos;
	u16 secoff;
	u16 secremain;	   
...
//擦除一个扇区的最少时间:150ms
void W25Q256_Erase_Sector(u32 Dst_Addr)   
{  
